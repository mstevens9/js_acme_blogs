...
const createElemWithText = (elem="p", textContent="", className) => {
    const textElem = document.createElement(elem);
    const textCont = document.createTextNode(textContent);
    textElem.appendChild(textCont);
    if (!className) return textElem;
...
const createPosts = async (postsData) => {
    ...
    for (const post of postsData) {
        ...
        let pPostId = createElemWithText('p', `Post ID: ${post.id}`);
...
const displayPosts = async (postsData) => {
    ...
    let element = postsData && postsData.length > 0
